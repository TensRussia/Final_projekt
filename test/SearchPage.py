...
class SearchPage:
    """Класс PageObject для взаимодействия
    с полем поиска городов/аэропортов.
    """
    ...
    def __init__(self, driver: WebDriver) -> None:
        ...
        self.empty_result_locator = (By.CSS_SELECTOR,
    ...
    def enter_search_query(self, query: str) -> None:
        ...
        element = self.wait.until(
            EC.element_to_be_clickable(self.input_locator)
            )
        element.clear()
    ...
    def get_dropdown_items_text(self) -> List[str]:
        ...
        try:
            items = self.wait.until(
                EC.presence_of_all_elements_located(self.dropdown_item_locator)
                )
    ...
    def select_dropdown_item_by_text(self, target_text: str) -> None:
        """Ищет элемент в дропдауне по частичному совпадению текста
        и кликает по нему.
        Args:
            target_text (str): Текст (или часть текста),
            который должен быть в элементе.
        """
        self.wait.until(
            EC.text_to_be_present_in_element(
                self.dropdown_item_locator, target_text)
                )
        items = self.wait.until(
            EC.presence_of_all_elements_located(self.dropdown_item_locator)
            )
        for item in items[0].find_elements(By.XPATH, './*'):
            if target_text.lower() in item.text.lower():
                item.click()
                return
        raise AssertionError(f"Элемент '{target_text}' не найден в списке")
    ...
    def get_empty_result_message(self) -> str:
        ...
        try:
            element = self.wait.until(
                EC.visibility_of_element_located(self.empty_result_locator)
                )
            return element.text
        except Exception:
            return ""
